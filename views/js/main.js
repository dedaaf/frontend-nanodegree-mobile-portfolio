...
var resizePizzas = function(size) {
    ...
    function changePizzaSizes(size) {
        ...
        sizeFactor = sizeSwitcher(size);
        newwidth = windowwidth * sizeFactor + 'px';

        divs.forEach(function(elem) { //iterate through all elements
            elem.style.width = newwidth; //set new width for elements
        });
...
var pizzaNumberID = 1; //send ID number to pizzaElementGenerator
...
function generateRandomPizzas() {
    ...
    for (var i = 2; i < 4; i++) {
        ...
        pizzaNumberID++;
        pizzasDiv.appendChild(pizzaElementGenerator(pizzaNumberID));

    }

}

function requestTick() { //if there is a scroll, a tick, update the positions, if not, use the same position
    if (!ticking) {
...
function updatePositions() {
    ...
    ticking = false;
    var currentScrollY = Top; //the currentScroll Y-axes position is equal to the earlier calculated Top-position
...
// runs isScroll (if there is a scroll) on scroll
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var amountPizzaColumn = Math.round(windowWidth / 240); //calculate the amount of pizzas in a column
    ...
    for (var i = 0; i < amountPizzas; i++) {
        ...
        elem.basicLeft = (i % amountPizzaColumn) * spaceBetween;
